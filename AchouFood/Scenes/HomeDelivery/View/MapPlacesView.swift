...
//  AchouFood
//
//  Created by Silvano Malfatti on 10/11/25.
//
...
extension MapPlacesView {
    func renderAnnotations(from places: [Place]) {
        mapView.removeAnnotations(mapView.annotations)
        guard !places.isEmpty else { return }
        let annotationsList = places.map { place in PlaceAnnotation(place: place) }
        mapView.addAnnotations(annotationsList)
        if annotationsList.count == 1, let annotation = annotationsList.first {
